...
from utils.role_checker import allowed_for_admin_user_executor
...
async def user_logout(request: Request, response: Response, user: User = Depends(allowed_for_admin_user_executor)):
...
async def get_user_data(user: User = Depends(allowed_for_admin_user_executor)):
    return user
